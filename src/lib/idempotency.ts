 ...
 * header is missing or outside 8–200 characters (`devplatform/src/server.ts`), and it
 ...
 * `devplatform/src/routeidempotency.test.ts`, and the service has a source-level test that
 ...
 *
 * `devplatform/src/server.ts`: a double-clicked "Create key" without the wrapper "mints
 ...
 * `devplatform/src/idempotency.ts`) and then makes three different decisions:
 *
 *   * same key, same request, work committed → the stored response is REPLAYED, `200` with
 *     `replayed: true` (`devplatform/src/server.ts`). **The secret is NOT in the stored
 *     response**, so a replay answers `secretKey: null`, which is the behaviour that makes the
 *     replay safe (`devplatform/src/server.ts`).
 ...
 *     `idempotency_in_flight` (`devplatform/src/server.ts`). Retry shortly, with the SAME
 *     key.
 *
...
/** How the service spells the two idempotency refusals. `devplatform/src/server.ts`. */
export const IN_FLIGHT_CODE = 'idempotency_in_flight'
export const KEY_REUSE_CODE = 'idempotency_key_reuse'

/** The header name, spelled as the service reads it — `devplatform/src/server.ts`. */
 ...
 * anyway (`devplatform/src/server.ts`), so this prefix is for humans, not for collision
 * avoidance.
 *
 ...
 *     (`devplatform/src/server.ts`) and `503 verifier_unavailable`, both of
