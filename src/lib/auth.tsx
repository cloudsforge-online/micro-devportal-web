 ...
 * token is verified against identity's JWKS (`devplatform/src/server.ts`), the caller's role in
 * an organisation is asked of identity PER REQUEST with the caller's own token forwarded
 * (`devplatform/src/server.ts`, `devplatform/src/membership.ts`), and a project or
 * organisation the caller may not see answers **404 rather than 403**
 * (`devplatform/src/server.ts`) so that ids are not enumerable across customers. This
 ...
 * routes outside it: `GET /v1/scopes` (`devplatform/src/server.ts`), `GET /v1/apps`
 * and `GET /v1/apps/:slug` read no credential at all. Sending an anonymous visitor to
 ...
 * built by `toPublicUser` at `identity/src/users.ts`; both were re-read against the source
 * for this repository rather than carried over.
 *
 * **The route citation names the FILE, not a line in it, and that is a correction.** It named a
 * thirteen-line range, and micro-identity's route table then moved twice in one afternoon, by
 * sixty-three lines and then by forty-six — so the range existed and registered somebody else's
 * route, which reads as verified and
 ...
 * membership (`identity/src/organisations.ts`), so the ROLE is on the wire. This app needs
 * both halves: the id is what `POST /v1/organisations` enrols
 * (`devplatform/src/server.ts`), and the role is what decides whether the attempt can possibly
 * succeed — devplatform re-asks identity for it and refuses anything below admin.
 *
 ...
 * organisation the user has nothing to do with — the exact thing `devplatform/src/server.ts`
 * exists to refuse.
 */
 ...
 *
 * Copied from `devplatform/src/membership.ts` — `owner` and `admin`, nothing else — and used
 * only to say so on screen. `devplatform/src/server.ts` is where it is decided, against
