...
export interface AppRoute {
   ...
   *
   * Read off the SERVICE rather than chosen. `GET /v1/scopes` (`devplatform/src/server.ts:604`),
   * `GET /v1/apps` (`:1022`) and `GET /v1/apps/:slug` (`:1027`) read no credential at all — their
...
export const ROUTES: readonly AppRoute[] = [
  ...
  //
  // `GET /v1/scopes` is served to anybody, and `devplatform/src/server.ts:600-602` says why: it "is
  ...
  // (`devplatform/src/server.ts:638`, `:644-645`).
  ...
  // `project:write` (`devplatform/src/server.ts:510`).
  ...
  // (`devplatform/src/applications.ts:201-204`), so nothing unreviewed can reach it by a caller
