...
export interface AppRoute {
   ...
   *
   * Read off the SERVICE rather than chosen. `GET /v1/scopes` (`devplatform/src/server.ts:712`),
   * `GET /v1/apps` (`:1263`) and `GET /v1/apps/:slug` (`:1291`) read no credential at all — their
...
export const ROUTES: readonly AppRoute[] = [
  ...
  //
  // `GET /v1/scopes` is served to anybody, and `devplatform/src/server.ts:708-710` says why: it "is
  ...
  // (`devplatform/src/server.ts:746`, `:752-753`).
  ...
  // `project:write` (`devplatform/src/server.ts:603`).
