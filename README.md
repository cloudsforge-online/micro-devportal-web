...
(`devplatform/src/server.ts`) — and **not one of the 35 `/v1` handlers contains a literal
`await authenticate(ctx, deps)`**. That call appears three times in the whole file and all three are
inside helpers. A boolean grep for the literal, which is what
...
| `POST` | `/v1/organisations` | `authenticateUser` + `permits(role, ADMIN_ROLES)` | — | `devplatform/src/server.ts` |
| `GET` | `/v1/organisations` | `authenticateUser` + `permits(role, READ_ROLES)` | — | `devplatform/src/server.ts` |
| `GET` | `/v1/organisations/:id` | `authoriseOrg` read | — | `devplatform/src/server.ts` |
| `GET` | `/v1/organisations/:id/projects` | `authoriseOrg` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/projects` | `authoriseOrg` write | **required** | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/projects/:id/service-accounts` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/service-accounts` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/projects/:id/keys` | `authoriseProject` write | **required** | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/keys` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `DELETE` | `/v1/keys/:id` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `PUT` | `/v1/projects/:id/quotas` | operator to raise or create, `authoriseProjectAs` write to lower | — | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/quotas` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/usage` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/projects/:id/webhook-endpoints` | `authoriseProject` write | **required** | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/webhook-endpoints` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/webhook-endpoints/:id/rotate-secret` | `authoriseProject` write | **required** | `devplatform/src/server.ts` |
| `POST` | `/v1/webhook-endpoints/:id/disable` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `POST` | `/v1/webhook-endpoints/:id/enable` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `DELETE` | `/v1/webhook-endpoints/:id` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `GET` | `/v1/webhook-endpoints/:id/deliveries` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/projects/:id/oauth-clients` | `authoriseProject` write | **required** | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/oauth-clients` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `DELETE` | `/v1/oauth-clients/:id` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `GET` | `/v1/apps` | **none** | — | `devplatform/src/server.ts` |
| `GET` | `/v1/apps/:slug` | **none** | — | `devplatform/src/server.ts` |
| `PUT` | `/v1/projects/:id/application` | `authoriseProject` write | — | `devplatform/src/server.ts` |
| `GET` | `/v1/projects/:id/application` | `authoriseProject` read | — | `devplatform/src/server.ts` |
| `POST` | `/v1/projects/:id/application/submit` | `authoriseProject` write | — | `devplatform/src/server.ts` |
...
`authoriseOrg` accepts a **user token only** and asks identity for the caller's
role on every request. `authoriseProject` accepts a user token **or an API key**, and a key
...
| --- | --- | --- | --- |
| `GET` | `/v1/keys/self` | `devplatform/src/server.ts` | The whoami for a **machine** credential. `authenticateKeyOnly` refuses anything that is not a `cfk_…` string, so the user JWT this bundle holds is a 403 — and satisfying it would mean a browser holding a live API key, which is the one thing this product exists to stop. It is the SDK's route, not the console's. |
| `GET` | `/v1/keys/:id` | `devplatform/src/server.ts` | It answers the identical `ApiKeySummary` (`devplatform/src/apikeys.ts`) that the project's key list already returns for every key, and this app has no per-key address. |
| `GET` | `/v1/apps/pending` | `devplatform/src/server.ts` | **An operator route, and this is a customer console.** It lists other customers' unpublished submissions, keyed on nothing the reader owns. A CloudsForge platform admin signing in here would get a 200, which is the argument for not drawing it rather than against: a control that works for one class of reader and 403s for every other reader of the same screen teaches the wrong thing about whose console this is. |
| `PUT` | `/v1/projects/:id/application/status` | `devplatform/src/server.ts` | **The route this repository asked for, declined here on purpose.** It closes the "nothing can approve a submitted application" finding, and it is an operator's: "a directory a developer can publish to unilaterally is a directory that eventually hosts a phishing page wearing this platform's chrome", and the OAuth consent screen is rendered from exactly that row. The submitting party must not hold the approving control. |
| `POST` | `/v1/events` | `devplatform/src/server.ts` | The internal inbox, HMAC-checked over the raw bytes before `JSON.parse`. A browser cannot hold that secret, and a bundle that shipped it would BE the revoke-anybody's-credentials endpoint the check prevents. |
...
from a browser: `deploy/gateway/dynamic/policy.yml` refuses `^/+internal(/|$)` at a priority
nothing can outrank.
...
Four routes return a credential and none returns it twice: an API key, a webhook signing
secret on creation and on rotation, and an OAuth client secret.
...
verbatim (`devplatform/src/server.ts`, duplicated as `SHOWN_ONCE` in `src/lib/format.ts` and
...
(`devplatform/src/server.ts`). So a replay answers `200` with `replayed: true` and the
...
requires the secret itself (`devplatform/src/migrations.ts`). It is still shown once, because
no route returns it (`devplatform/src/webhooks.ts`). `WEBHOOK_SECRET_NOTE` says exactly that and
...
(`devplatform/src/server.ts`). The eleven exempt mutations each name the mechanism that
...
`devplatform/src/routeidempotency.test.ts`.
...
**The gate is not the security boundary.** `devplatform` verifies the bearer itself, asks
identity for the caller's role per request, and answers **404 rather than 403** for an
organisation or project the caller may not see so that ids are not enumerable across
...
`developers` devPort **3012** (`ui/packages/ui/src/surfaces.ts`) and this bundle derives its API
base from the registry, while `devplatform` binds **4000** (`devplatform/src/env.ts`,
...
`[data-cf-product='developers']` (`ui/packages/ui/src/tokens.css`), set statically on `<html>` so
...
(`brand/README.md`, `brand/README.md`, `brand/plan.ts`). The og card is shipped
...
The registry carries `markId: null` for this surface (`ui/packages/ui/src/surfaces.ts`), so
...
wallet, market, mint and worlds and for nothing else, so a key issued through this console works
against no public host." `deploy/gateway/dynamic/public-api.yml` now declares `cf-api-devplatform`,
matching `/v1/apps`, `/v1/keys`, `/v1/oauth-clients`, `/v1/organisations`, `/v1/projects`,
`/v1/scopes` and `/v1/webhook-endpoints` — quotas and usage sit under `/v1/projects` — forwarded
unchanged to `devplatform`, and the blackhole catch-all this finding described has been deleted.
`deploy/compose/docker-compose.estate.yml` declares the service and its migration job.

The finding went on being rendered as a warning long after it stopped being true, and the only
thing that could have caught it was its own citation: it named a `docker-compose.slice.yml` under
`deploy/compose`, which micro-deploy has deleted — not written out as a path here, because a
citation to a file that does not exist is the subject of this paragraph. The sweep in
`test/citations.test.ts` did not see it, because that sweep only looked at citations carrying a
LINE NUMBER — and a citation to a file that no longer exists is exactly the shape a line-number
check cannot have an opinion about. It now checks every cited file and forbids the line.
...
(`devplatform/src/server.ts`); that is a status-code change on shipped routes and rightly
