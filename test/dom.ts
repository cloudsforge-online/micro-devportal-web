...
export interface Wire {
  ...
  /**
   * The path as the BROWSER sent it, mount and all — `/developers/v1/…`.
   *
...
export async function mount(element: ReactElement, options: MountOptions = {}): Promise<Screen> {
  // ── THE PAGE IS AT `<apex>/developers` SINCE 2026-08-19, AND THIS DEFAULT IS WHERE IT LANDS ───────
  //
  ...
  const url = options.url ?? 'https://cloudsforge.online/developers'
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    //
    // Wave 3b moved this surface to `<apex>/developers`, so `apiBaseFor()` answers `/developers` and every
    // request the app makes is now `/developers/v1/…` rather than `/v1/…`. The scenarios key their
    ...
    // exactly what `micro-devplatform` receives, because the gateway's stripPrefix removes `/developers` before
