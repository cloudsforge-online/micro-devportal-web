...
describe('auth callback', () => {
   ...
   * `POST /auth/handoff/redeem` (in `identity/src/server.ts`; the line this used to name, :1084,
   * is now the middle of a different handler), so every SSO callback in the
  ...
  it('strips the code from the address bar BEFORE the redemption is sent', async () => {
    browser = installWindow('https://developers.cloudsforge.online/reports?tab=1#cf_code=abc123&view=grid')
  ...
  it('redeems at the route identity serves, on identity’s host', async () => {
    browser = installWindow('https://developers.cloudsforge.online/#cf_code=abc123')
    stub = installFetch(identityRedemption, browser.trace)
  ...
  it('still strips the code when the redemption fails', async () => {
    ...
    browser = installWindow('https://developers.cloudsforge.online/#cf_code=dead')
  ...
  it('does nothing to a URL that carries no code', async () => {
    browser = installWindow('https://developers.cloudsforge.online/overview#section-2')
    let calls = 0
    stub = installFetch(() => {
      calls += 1
      return json(200, {})
